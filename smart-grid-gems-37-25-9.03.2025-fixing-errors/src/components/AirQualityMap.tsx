...
import { fetchAirlyData, fetchGIOSData, fetchAQICNData } from './pomerania/map/MapContainer';
...
export const AirQualityMap = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [airlyData, giosData, aqicnData] = await Promise.all([
          fetchAirlyData(),
          fetchGIOSData(),
          fetchAQICNData()
        ]);
        
        setSensors([...airlyData, ...giosData, ...aqicnData]);
      } catch (error) {
